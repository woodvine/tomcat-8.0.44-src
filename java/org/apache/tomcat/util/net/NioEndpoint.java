 ...
 * </ul>
 *  stopInternal:用到CountDownLatch来控制，等待子线程结束后，主线程再shutdownExecutor.
...
public class NioEndpoint extends AbstractEndpoint<NioChannel> {
    ...
    /**
     * The size of the OOM parachute.2G
     */
     ...
     * Timestap to be recoreded
     */
    private long lastParachuteCheck = System.currentTimeMillis();

    /**
     * 一个线程等待其他线程任务结束时调用：主线程await，其他线程countDown直到0时，主线程被唤醒。
     */
     ...
     * Acceptor put PollerEvent to it
     */
        ...
        try {
            ...
            if ( channel == null ) {
                ...
                if (sslContext != null) {
                    ...
                    System.out.println("SecureNioChannel is created.");
    ...
    /**
     * dispatch是标识是否转发给线程池工作线程处理的意思，并不是页面的重定向之意
     * @return
     */
    ...
    /**
     * 将Socket注册到Selector上去：通常是OPT_REGISTER和OPT_READ
     * 被当作普通类型来使用的，直接调用其run()方法完成处理的，而不是线程任务。没有必要实现Runnable接口。
    ...
    public static class PollerEvent implements Runnable {
        ...
        public PollerEvent(NioChannel ch, KeyAttachment k, int intOps) {
            System.out.println("PollerEvent construtcor intOps:"+intOps);
            reset(ch, k, intOps);
        }

        public void reset(NioChannel ch, KeyAttachment k, int intOps) {
            System.out.println("PollerEvent reset intOps:"+intOps);
            socket = ch;
            interestOps = intOps;
            key = k;
        }
        ...
        public void run() {
            if ( interestOps == OP_REGISTER ) {
                ...
                System.out.println("这里的事件不是默认的注册事件，而是其他事件："+interestOps);
    ...
    public class Poller implements Runnable {
        ...
        public boolean events() {
            ...
            while ( (pe = events.poll()) != null ) {
                result = true;
                try {
                    System.out.println("pe before run :"+pe);
                    pe.run();
                    pe.reset();
                    System.out.println("pe fater reset :"+pe);
                    if (running && !paused) {
                        eventCache.push(pe);
                    }
                } catch ( Throwable x ) {
...
//            System.out.println("register :"+r);
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    while (iterator != null && iterator.hasNext()) {
                        ...
                        System.out.println("sk opt:"+sk.interestOps());
            ...
            //到达此处，说明程序在执行stopInternal流程，Poller停止时countDown，
            // 直到所有CountDownLatch降到0，唤醒主线程的await操作。
            stopLatch.countDown();
        }
                    ...
                    if (sk.isReadable() || sk.isWritable() ) {
                        if ( attachment.getSendfileData() != null ) {
                            ...
                            if ( isWorkerAvailable() ) {
                                //更改当前SelectionKey的事件为0，即取消该事件的注册，下一次处理时将不再关注这种事件
                                unreg(sk, attachment, sk.readyOps());
                                boolean closeSocket = false;
                                // Read goes before write
                                if (sk.isReadable()) {
                                    System.out.println("Process Key go into closeSocket=false，and readable");
                                    if (!processSocket(attachment, SocketStatus.OPEN_READ, true)) {
                                        closeSocket = true;
                                    }
                                }
                                if (!closeSocket && sk.isWritable()) {
                                    System.out.println("Process Key go into closeSocket=false，and writable");
        ...
        //~readyOps是取非操作：即注销上次的事件，避免下次Selector轮询时再处理这种事件。
        ...
        private void doRun() {
            ...
            try {
                ...
                if (handshake == 0) {
                    ...
                    //完全没有这种情况产生的
                    System.out.println("handshake不是0，或-1，这个诡异的分支是不能产生的!");
